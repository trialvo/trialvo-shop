...
hbsPackage.registerHelper('or', function () {
    const args = Array.prototype.slice.call(arguments, 0, -1);
    return args.some(Boolean);
});


exports. sendPaymentMail = async (connection, mailObj) => {
  const { name, email ,order,status} = mailObj;
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `${BRAND_NAME} Payment ${status === 'success' ? 'Successful' : status === 'failed' ? 'Failed' : 'Cancelled'}`,
      template: "payment",
      context: {
        name: name || "User",
        BRAND_NAME,
