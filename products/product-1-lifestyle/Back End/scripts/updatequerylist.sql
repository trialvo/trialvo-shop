...
ALTER TABLE `product_images`
  DROP FOREIGN KEY IF EXISTS `fk_pi_color`,
  DROP KEY IF EXISTS `idx_pi_color`;
...
ALTER TABLE `order_items`
  ADD COLUMN IF NOT EXISTS `weight_kg` DECIMAL(8,3) NOT NULL DEFAULT 0.000
    COMMENT 'SKU weight snapshot (kg) at time of order placement'
    AFTER `stock_adjusted`;

ALTER TABLE `orders`
  ADD COLUMN IF NOT EXISTS `weight_kg_total`     DECIMAL(10,3) NOT NULL DEFAULT 0.000
    COMMENT 'Total order weight (kg)' AFTER `discount_total`,
  ADD COLUMN IF NOT EXISTS `weight_extra_charge` DECIMAL(10,2) NOT NULL DEFAULT 0.00
...
ALTER TABLE `orders`
  ADD COLUMN IF NOT EXISTS `bulk_discount_total`  DECIMAL(12,2) NOT NULL DEFAULT 0.00 AFTER `discount_total`,
  ADD COLUMN IF NOT EXISTS `combo_discount_total` DECIMAL(12,2) NOT NULL DEFAULT 0.00 AFTER `bulk_discount_total`,
  ADD COLUMN IF NOT EXISTS `cart_wide_discount`   DECIMAL(12,2) NOT NULL DEFAULT 0.00 AFTER `combo_discount_total`,
  ADD COLUMN IF NOT EXISTS `weight_kg_total`      DECIMAL(10,3) NOT NULL DEFAULT 0.000 AFTER `cart_wide_discount`,
  ADD COLUMN IF NOT EXISTS `weight_extra_charge`  DECIMAL(12,2) NOT NULL DEFAULT 0.00  AFTER `weight_kg_total`;

ALTER TABLE `order_items`
  ADD COLUMN IF NOT EXISTS `bulk_rule_id`           BIGINT UNSIGNED DEFAULT NULL AFTER `coupon_discount`,
  ADD COLUMN IF NOT EXISTS `bulk_discount_applied`  DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER `bulk_rule_id`,
  ADD COLUMN IF NOT EXISTS `combo_rule_id`          BIGINT UNSIGNED DEFAULT NULL AFTER `bulk_discount_applied`,
  ADD COLUMN IF NOT EXISTS `combo_discount_applied` DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER `combo_rule_id`;
...
ALTER TABLE `orders`
  ADD COLUMN IF NOT EXISTS `sku_discount_total` DECIMAL(12,2) NOT NULL DEFAULT 0.00
...
ALTER TABLE user_addresses ADD COLUMN IF NOT EXISTS location_mapping_id INT NULL DEFAULT NULL AFTER zip_code;
ALTER TABLE order_addresses ADD COLUMN IF NOT EXISTS location_mapping_id INT NULL DEFAULT NULL AFTER zip_code;
-- END V2-029
...
ALTER TABLE location_mappings
  ADD COLUMN IF NOT EXISTS paperfly_thana_id INT DEFAULT NULL
...
ALTER TABLE location_mappings DROP INDEX IF EXISTS uniq_location;

-- Add per-courier unique keys for proper deduplication
ALTER TABLE location_mappings
  ADD UNIQUE KEY IF NOT EXISTS uniq_pathao_area (pathao_area_id),
  ADD UNIQUE KEY IF NOT EXISTS uniq_steadfast_id (steadfast_id);

-- Add performance indexes
ALTER TABLE location_mappings
  ADD INDEX IF NOT EXISTS idx_lm_city (city_name),
  ADD INDEX IF NOT EXISTS idx_lm_pathao_city (pathao_city_id);

-- Add index on location_mapping_id in address tables for dispatch JOIN performance
ALTER TABLE order_addresses
  ADD INDEX IF NOT EXISTS idx_oa_location_mapping (location_mapping_id);
ALTER TABLE user_addresses
  ADD INDEX IF NOT EXISTS idx_ua_location_mapping (location_mapping_id);
...
ALTER TABLE `orders`
  ADD COLUMN IF NOT EXISTS `assigned_to_admin_id` INT NULL AFTER `memo`,
  ADD COLUMN IF NOT EXISTS `assigned_by_admin_id` INT NULL AFTER `assigned_to_admin_id`,
  ADD COLUMN IF NOT EXISTS `assignment_method` ENUM('auto','manual','redistribute') NULL AFTER `assigned_by_admin_id`,
  ADD COLUMN IF NOT EXISTS `assigned_at` TIMESTAMP NULL AFTER `assignment_method`,
  ADD KEY IF NOT EXISTS `idx_orders_assigned_admin` (`assigned_to_admin_id`);

-- 5. Add allow_handle_unassigned_order to per-admin permissions
ALTER TABLE `admin_notification_permissions`
  ADD COLUMN IF NOT EXISTS `allow_handle_unassigned_order` TINYINT(1) NOT NULL DEFAULT 1
...
ALTER TABLE `admin_notification_permissions`
  ADD COLUMN IF NOT EXISTS `contact_notification_email`         TINYINT(1) NOT NULL DEFAULT '1' AFTER `personal_notification_firebase_push`,
  ADD COLUMN IF NOT EXISTS `contact_notification_sms`           TINYINT(1) NOT NULL DEFAULT '0' AFTER `contact_notification_email`,
  ADD COLUMN IF NOT EXISTS `contact_notification_firebase_push` TINYINT(1) NOT NULL DEFAULT '1' AFTER `contact_notification_sms`,
  ADD COLUMN IF NOT EXISTS `report_notification_email`          TINYINT(1) NOT NULL DEFAULT '1' AFTER `contact_notification_firebase_push`,
  ADD COLUMN IF NOT EXISTS `report_notification_sms`            TINYINT(1) NOT NULL DEFAULT '0' AFTER `report_notification_email`,
  ADD COLUMN IF NOT EXISTS `report_notification_firebase_push`  TINYINT(1) NOT NULL DEFAULT '1' AFTER `report_notification_sms`;

-- 2. Extend contact_messages (assignment columns)
ALTER TABLE `contact_messages`
  ADD COLUMN IF NOT EXISTS `assigned_to_admin_id` INT DEFAULT NULL AFTER `status`,
  ADD COLUMN IF NOT EXISTS `assigned_by_admin_id` INT DEFAULT NULL AFTER `assigned_to_admin_id`,
  ADD COLUMN IF NOT EXISTS `assigned_at`          TIMESTAMP NULL DEFAULT NULL AFTER `assigned_by_admin_id`;
ALTER TABLE `contact_messages` ADD INDEX IF NOT EXISTS `idx_cm_assigned` (`assigned_to_admin_id`);
...
-- 1. Add assignment columns to contact_messages (if not already present from V2-036)
ALTER TABLE contact_messages
  ADD COLUMN IF NOT EXISTS assigned_to_admin_id  INT(11)   DEFAULT NULL  AFTER is_replied,
  ADD COLUMN IF NOT EXISTS assigned_by_admin_id  INT(11)   DEFAULT NULL  AFTER assigned_to_admin_id,
  ADD COLUMN IF NOT EXISTS assigned_at           DATETIME  DEFAULT NULL  AFTER assigned_by_admin_id;

ALTER TABLE contact_messages
  ADD KEY IF NOT EXISTS idx_cm_assigned (assigned_to_admin_id);
...
ALTER TABLE `guest_orders`
  ...
  DROP COLUMN IF EXISTS `email_otp_exp`,
...
ALTER TABLE `announcement_zones`
  ADD COLUMN IF NOT EXISTS `location_mapping_id` INT NULL
    COMMENT 'FK-like ref to location_mappings.id for area-level targeting'
    AFTER `announcement_id`,
  ADD COLUMN IF NOT EXISTS `area_name` VARCHAR(150) NULL
    AFTER `city_name`,
  ADD COLUMN IF NOT EXISTS `area_name_normalized` VARCHAR(170) NULL
...
-- ============================================================
-- [V2-035] announcements-add-channel-column
-- Date: 2026-04-17
-- Summary: Add `channel` column to `announcements` table to support per-announcement
--          dispatch channel selection (email | sms | both).
--          The controller (announcement.js) already reads/writes this column; the column
--          was missing from the DB, causing "Unknown column 'channel' in 'field list'" errors
--          on PUT /api/v1/admin/announcement/:id whenever channel was included in the payload.
-- Preconditions:
--   1) Apply to all environments (local + production).
--   2) No code changes needed — controller is already correct after the datetime fix.

-- START V2-035
ALTER TABLE `announcements`
  ADD COLUMN `channel` ENUM('email', 'sms', 'both')
...
--till here is diployed

-- [V2-036] mega-sale-dedicated-tables
-- Date: 2026-04-18
-- Summary: Migrate Mega Sale configuration from permission_config (storefront_visibility section)
--          to dedicated tables: mega_sale_settings (single-row global config) and
--          mega_sale_products (per-SKU entries). Supports SKU-level granularity so admins
--          can highlight specific variations. Removes the hard product limit in favor of
--          server-side pagination.
-- Preconditions:
--   1) Apply to all environments (local + production).
--   2) Deploy new API controller (controllers/megasale.js) and updated storefront-visibility
--      endpoint BEFORE running this migration in production.

-- START V2-036
START TRANSACTION;

-- 1. Create singleton global settings table
CREATE TABLE `mega_sale_settings` (
  `id`                 TINYINT UNSIGNED NOT NULL DEFAULT 1,
  `is_active`          TINYINT(1)       NOT NULL DEFAULT 0 COMMENT 'Master toggle: show mega sale page on storefront',
  `campaign_end_at`    DATETIME         NULL     COMMENT 'Global campaign countdown end time (banner timer)',
  `default_end_at`     DATETIME         NULL     COMMENT 'Fallback per-product countdown (used when product has no individual timer)',
  `updated_by_admin`   INT              NULL,
  `created_at`         TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`         TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  COMMENT='Single-row global Mega Sale configuration (V2-036)';

INSERT INTO `mega_sale_settings` (`id`) VALUES (1)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`);

-- 2. Create per-SKU mega sale entries
CREATE TABLE `mega_sale_products` (
  `id`              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `product_sku_id`  INT             NOT NULL COMMENT 'FK to product_skus.id (specific variation)',
  `is_active`       TINYINT(1)      NOT NULL DEFAULT 1 COMMENT 'Per-SKU toggle',
  `end_at`          DATETIME        NULL     COMMENT 'Per-SKU countdown override (NULL = inherit global default_end_at)',
  `serial`          INT             NOT NULL DEFAULT 0 COMMENT 'Display order on storefront',
  `created_at`      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_mega_sale_sku` (`product_sku_id`),
  KEY `idx_msp_active_serial` (`is_active`, `serial`),
  CONSTRAINT `fk_msp_sku`
    FOREIGN KEY (`product_sku_id`) REFERENCES `product_skus` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Per-SKU Mega Sale entries with individual timer overrides (V2-036)';

-- 3. Migrate existing data from permission_config → mega_sale_settings
-- is_active
UPDATE `mega_sale_settings` ms
SET ms.`is_active` = (
  SELECT CASE WHEN pc.`value` = 'true' THEN 1 ELSE 0 END
  FROM `permission_config` pc
  WHERE pc.`section` = 'storefront_visibility'
    AND pc.`scope`   = 'default'
    AND pc.`key_name` = 'show_megasale'
  LIMIT 1
)
WHERE ms.`id` = 1
  AND EXISTS (
    SELECT 1 FROM `permission_config`
    WHERE `section` = 'storefront_visibility'
      AND `scope`   = 'default'
      AND `key_name` = 'show_megasale'
  );

-- campaign_end_at
UPDATE `mega_sale_settings` ms
SET ms.`campaign_end_at` = (
  SELECT NULLIF(TRIM(pc.`value`), '')
  FROM `permission_config` pc
  WHERE pc.`section` = 'storefront_visibility'
    AND pc.`scope`   = 'default'
    AND pc.`key_name` = 'megasale_campaign_end_at'
  LIMIT 1
)
WHERE ms.`id` = 1;
...
-- default_end_at
UPDATE `mega_sale_settings` ms
SET ms.`default_end_at` = (
  SELECT NULLIF(TRIM(pc.`value`), '')
  FROM `permission_config` pc
  WHERE pc.`section` = 'storefront_visibility'
    AND pc.`scope`   = 'default'
    AND pc.`key_name` = 'megasale_product_end_at'
  LIMIT 1
)
WHERE ms.`id` = 1;

-- NOTE: megasale_product_ids and megasale_product_timers migration requires
-- application-level string parsing (comma-separated IDs, key=value pairs).
-- These will be migrated by the new controller on first-run if needed,
-- or manually via the admin panel.

-- 4. Delete old storefront_visibility rows from permission_config
DELETE FROM `permission_config`
WHERE `section` = 'storefront_visibility'
  AND `scope`   = 'default';
...
-- ===========================================================================
-- Restructures mega_sale_products from SKU-level to product-level.
-- Adds mega_sale_sku_overrides for individual SKU exclusions/timers.
-- Removes default_end_at (campaign_end_at is the only global fallback).
-- ===========================================================================

START TRANSACTION;

-- 1. Drop default_end_at from settings (campaign_end_at is sufficient)
ALTER TABLE `mega_sale_settings` DROP COLUMN `default_end_at`;

-- 2. Create new product-level table
CREATE TABLE IF NOT EXISTS `mega_sale_products_new` (
  `id`          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- 3. Migrate existing SKU-level data → product-level (group by product_id)
INSERT INTO `mega_sale_products_new` (`product_id`, `is_active`, `end_at`, `serial`, `created_at`)
SELECT
  ps.product_id,
  MAX(msp.is_active),
  MAX(msp.end_at),
...
JOIN `product_skus` ps ON ps.id = msp.product_sku_id
GROUP BY ps.product_id
ON DUPLICATE KEY UPDATE `product_id` = VALUES(`product_id`);

-- 4. Drop old SKU-level table and rename new one
DROP TABLE IF EXISTS `mega_sale_products`;
ALTER TABLE `mega_sale_products_new` RENAME TO `mega_sale_products`;

-- 5. Create SKU overrides table
...
-- Date: 2026-04-21
-- Summary: Add report_images table to support image attachments
--          on report submissions (user) and report replies (admin).
--          reply_id NULL = attached to report; non-NULL = attached to a reply.
-- Preconditions:
--   1) V2-036 must be applied (reports + report_replies tables).
--   2) Deploy updated controllers/report.js and helpers/img.js first.
-- ============================================================
-- START V2-049
...
-- ============================================================
-- [V2-050] product-review-and-rating-system
-- Date: 2026-04-22
-- Summary: Product Review & Rating System.
--   Creates:
--     * product_reviews — user reviews linked to order items
--     * review_images   — image attachments for reviews and replies
--     * review_replies  — admin responses to reviews
--   Alters:
--     * products — adds avg_rating (DECIMAL 3,2) and review_count (INT)
--                  for denormalized fast listing queries.
--
--   Business rules:
--     - One review per order_item (user can review same product
--       across different orders).
--     - Editable until admin replies; then locked.
--     - @seller mention auto-creates report in distribution pool.
--     - Soft-delete via deleted_at on reviews and replies.
--     - avg_rating and review_count recalculated on every write.
--
-- Preconditions:
--   1) V2-049 must be applied (report_images).
--   2) Deploy updated controllers/review.js, helpers/img.js,
--      and mail-templates/reviewreply.js first.
-- ============================================================
-- START V2-050

-- 1. Add denormalized rating cache columns to products
ALTER TABLE `products`
  ADD COLUMN IF NOT EXISTS `avg_rating` DECIMAL(3,2) NOT NULL DEFAULT 0.00
    COMMENT 'Cached average star rating (1 decimal, recalculated on review write)'
    AFTER `sell_count`,
  ADD COLUMN IF NOT EXISTS `review_count` INT UNSIGNED NOT NULL DEFAULT 0
    COMMENT 'Cached count of visible (non-hidden, non-deleted) reviews'
    AFTER `avg_rating`;

ALTER TABLE `products`
  ADD KEY IF NOT EXISTS `idx_products_avg_rating` (`avg_rating`);
...
CREATE TABLE IF NOT EXISTS `product_reviews` (
  ...
  `mentions_seller` TINYINT(1)      NOT NULL DEFAULT 0 COMMENT 'True if @seller found in review_text',
  `report_id`       BIGINT UNSIGNED DEFAULT NULL COMMENT 'FK to reports.id when @seller mention auto-created a report',
  `status`          ENUM('published','hidden','flagged') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'published',
  `created_at`      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted_at`      TIMESTAMP       NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_review_order_item` (`order_item_id`) COMMENT 'One review per order item',
  KEY `idx_pr_product` (`product_id`),
  KEY `idx_pr_user` (`user_id`),
  KEY `idx_pr_rating` (`rating`),
  KEY `idx_pr_status` (`status`),
  KEY `idx_pr_deleted` (`deleted_at`),
  ...
  CONSTRAINT `fk_pr_order_item` FOREIGN KEY (`order_item_id`) REFERENCES `order_items`  (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Product reviews linked to verified purchases (V2-050)';
...
CREATE TABLE IF NOT EXISTS `review_replies` (
  ...
  `deleted_at`  TIMESTAMP       NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_rvr_review` (`review_id`),
  KEY `idx_rvr_admin`  (`admin_id`),
  CONSTRAINT `fk_rvr_review` FOREIGN KEY (`review_id`) REFERENCES `product_reviews` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_rvr_admin`  FOREIGN KEY (`admin_id`)  REFERENCES `admins`           (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Admin replies to product reviews (V2-050)';

-- 4. Create review_images table (shared by reviews and replies)
CREATE TABLE IF NOT EXISTS `review_images` (
  `id`          INT UNSIGNED    NOT NULL AUTO_INCREMENT,
  `review_id`   BIGINT UNSIGNED NOT NULL,
  `reply_id`    BIGINT UNSIGNED DEFAULT NULL COMMENT 'NULL = review-level image; non-NULL = reply attachment',
  `image_path`  VARCHAR(500)    NOT NULL,
  `serial`      TINYINT UNSIGNED NOT NULL DEFAULT 1,
  `created_at`  TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_review_images_review` (`review_id`),
  KEY `idx_review_images_reply`  (`reply_id`),
  CONSTRAINT `fk_rvi_review` FOREIGN KEY (`review_id`) REFERENCES `product_reviews` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_rvi_reply`  FOREIGN KEY (`reply_id`)  REFERENCES `review_replies`   (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Image attachments for product reviews and admin replies (V2-050)';

-- 5. Register review audit actions
INSERT IGNORE INTO `audit_actions` (`action_key`, `display_name`) VALUES
('REVIEW_REPLY',       'Reply to Review'),
('REVIEW_PIN_TOGGLE',  'Toggle Review Pin'),
('REVIEW_HIDE_TOGGLE', 'Toggle Review Hide'),
('REVIEW_DELETE',      'Delete Review');

INSERT IGNORE INTO `user_audit_actions` (`action_key`, `display_name`, `category`) VALUES
('SUBMIT_REVIEW', 'Submit Product Review', 'ORDER');

-- 6. Expand notification_histories category enum for review_reply
ALTER TABLE `notification_histories`
  MODIFY COLUMN `category`
  ENUM(
    'order_status',
...
-- ═══════════════════════════════════════════════════════════════════════════
-- V2-051: Allow comment-only reviews (no purchase required)
-- ═══════════════════════════════════════════════════════════════════════════

-- Make order/rating nullable so customers can leave comments without purchase
ALTER TABLE `product_reviews`
  ...
  MODIFY COLUMN `rating`        TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '1-5 stars, NULL for comment-only';

-- Drop the old unique constraint and FK that required order_item_id NOT NULL
ALTER TABLE `product_reviews` DROP FOREIGN KEY IF EXISTS `fk_pr_order`;
ALTER TABLE `product_reviews` DROP FOREIGN KEY IF EXISTS `fk_pr_order_item`;
ALTER TABLE `product_reviews` DROP INDEX IF EXISTS `uniq_review_order_item`;

-- Re-add unique constraint only for rated reviews (non-null order_item_id)
-- MySQL doesn't enforce UNIQUE on NULL values, so multiple NULLs are allowed
ALTER TABLE `product_reviews`
  ADD UNIQUE KEY `uniq_review_order_item` (`order_item_id`);

-- Re-add FK constraints with nullable references
ALTER TABLE `product_reviews`
  ADD CONSTRAINT `fk_pr_order`      FOREIGN KEY (`order_id`)      REFERENCES `orders`      (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pr_order_item` FOREIGN KEY (`order_item_id`) REFERENCES `order_items` (`id`) ON DELETE SET NULL;

-- Add a unique index to prevent duplicate comment-only entries per user per product
-- (one comment without purchase per product per user)
CREATE UNIQUE INDEX IF NOT EXISTS `uniq_comment_per_user_product`
  ON `product_reviews` (`product_id`, `user_id`, (IFNULL(`order_item_id`, 0)))
  COMMENT 'Prevents duplicate comment-only reviews';
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Summary: Add client_config (JSON) and vapid_key columns to firebase_push_credentials
--          so the admin panel can manage Firebase web app config + VAPID key from the UI.
--          A new public endpoint GET /config/firebase-client-config serves these values
--          to both shop and admin panels at runtime.
-- ═══════════════════════════════════════════════════════════════════════════

ALTER TABLE `firebase_push_credentials`
  ADD COLUMN `client_config` JSON NULL DEFAULT NULL COMMENT 'Firebase web app config (apiKey, authDomain, projectId, etc.)' AFTER `credential_json`,
  ADD COLUMN `vapid_key` VARCHAR(200) NULL DEFAULT NULL COMMENT 'VAPID public key for Web Push' AFTER `client_config`;

-- END V2-052

-- ═══════════════════════════════════════════════════════════════════════════
-- V2-053  Order Type ENUM Refactoring — Separate Admin-Created Orders
-- ═══════════════════════════════════════════════════════════════════════════
-- Date: 2026-05-03
-- Summary: Expand orders.order_type ENUM from ('regular','guest','single_page')
--          to ('regular','guest','admin_regular','admin_stranger','single_page').
--          Backfill existing admin-created orders using the created_by_admin flag.
-- Preconditions:
--   1) Run in maintenance window.
--   2) Deploy code changes to admin_order.js, order.js, admin_dashboard.js,
--      admin panel TypeScript types, and shop frontend OrderType union BEFORE
--      or alongside this migration.
-- ═══════════════════════════════════════════════════════════════════════════
...
ALTER TABLE `orders` MODIFY COLUMN `order_type`
  ENUM('regular','guest','admin_regular','admin_stranger','single_page')
  NOT NULL DEFAULT 'regular';

-- 2. Backfill existing admin-created orders
UPDATE `orders` SET `order_type` = 'admin_regular'
  WHERE `order_type` = 'regular' AND `created_by_admin` IS NOT NULL;

UPDATE `orders` SET `order_type` = 'admin_stranger'
  WHERE `order_type` = 'guest' AND `created_by_admin` IS NOT NULL;

COMMIT;
-- END V2-053

-- ═══════════════════════════════════════════════════════════════════════════
-- V2-054  Single Product Page — Product Flag Column
-- ═══════════════════════════════════════════════════════════════════════════
-- Date: 2026-05-03
-- Summary: Add `products.has_single_product_page` boolean column to control
--          whether a product has a dedicated single product page for direct
--          ordering (no-login checkout with OTP verification).
-- Preconditions:
--   1) Deploy product.js code changes (toggleSingleProductPage, getSinglePageData)
--      before or alongside this migration.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- V2-055  Single Product Page — Session Table for OTP Verification
-- ═══════════════════════════════════════════════════════════════════════════
-- Date: 2026-05-03
-- Summary: Creates `single_page_sessions` table used by the no-login
--          checkout flow to store phone/email OTP verification state.
-- ═══════════════════════════════════════════════════════════════════════════

-- START V2-055
CREATE TABLE IF NOT EXISTS `single_page_sessions` (
