...
const getApiMessage = (data?: ApiError | null) =>
  (data?.error || data?.message || "").trim();

const isVerifiedAccountConflict = (
  status: number | undefined,
  data?: ApiResponse<SignUpResponse>,
) => {
  const message = getApiMessage(data).toLowerCase();

  return (
    status === 409 ||
    ((status === 404 || data?.flag === 404) &&
      message.includes("already") &&
      (message.includes("verified") || message.includes("exist")))
  );
};
...
class AuthService {
  async signIn(data: SignInValues): Promise<ApiResponse<SignInResponse>> {
    try {
      ...
      // Return email-unverified responses so the UI can show a targeted
      // verification notice. Other 403s still use the generic error path.
  ...
  async signUp(data: SignUpValues): Promise<ApiResponse<SignUpResponse>> {
    try {
      ...
      // The backend currently reports a verified-account signup conflict as
      // flag 404. Normalize it so UI code handles one typed conflict branch.
      const axiosErr = err as {
        response?: { status?: number; data?: ApiResponse<SignUpResponse> };
      };
      if (
        axiosErr.response?.data &&
        isVerifiedAccountConflict(
          axiosErr.response.status,
          axiosErr.response.data,
        )
      ) {
        const message =
          getApiMessage(axiosErr.response.data) ||
