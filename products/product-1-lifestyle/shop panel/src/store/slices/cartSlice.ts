...
/**
 * SKU-based cart slice matching the reference shop model.
...
const initialState: CartState = {
  ...
  weightExtraPerKg: 0,
  hydrated: false,
  appliedCoupon: { coupon: null, discount: 0 },
  guestId: null,
  buyNowId: null,
  isCartOpen: false,
};

type AddItemPayload = Omit<CartItem, "id"> & { id?: string; overrideQuantity?: boolean; /** @deprecated Use title */ name?: string };
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    initCart(state) {
      ...
      state.discount = loadCartDiscount();
      state.deliveryCharge = loadCartDeliveryCharge();
      state.appliedCoupon = loadCartCoupon();
      state.guestId = loadGuestId();
      state.buyNowId = loadBuyNowId();
      state.hydrated = true;
    },
    ...
    editItem(state, action: PayloadAction<EditItemPayload>) {
      ...
      if (sizeChanged || colorChanged) {
        ...
        if (existingItemIndex !== -1 && existingItemIndex !== itemIndex) {
          ...
          const mergedStock = typeof updates.stock === "number" ? updates.stock : existingItem.stock ?? currentItem.stock;
          ...
          const mergedItem: CartItem = {
            ...
            quantity: Math.min(maxQty, existingItem.quantity + (updates.quantity || currentItem.quantity)),
            stock: mergedStock,
          };
          ...
          const maxQty = getMaxQty(typeof updates.stock === "number" ? updates.stock : currentItem.stock);
    ...
    setQuantity(state, action: PayloadAction<SetQtyPayload>) {
      ...
      if (!item) return;
      item.quantity = Math.min(getMaxQty(item.stock), Math.max(1, action.payload.quantity));
    },
    ...
    syncCartItems(state, action: PayloadAction<{ id: string; price: number; originalPrice: number; discount: number; stock: number; weight_kg?: number; free_delivery?: boolean }[]>) {
      const updatesMap = new Map<string, (typeof action.payload)[number]>();
...
export const { selectCartItems, selectTotalItems, selectSubtotal } = cartSlice.selectors;
