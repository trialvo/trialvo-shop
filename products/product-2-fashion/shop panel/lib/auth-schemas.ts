...
const emailOrMobileSchema = z
  ...
  .refine((val) => emailSchema.safeParse(val).success || bdMobileSchema.safeParse(val).success, {
...
export const signUpSchema = z.object({
  ...
  last_name: z.string().trim().min(2, "Last name must be at least 2 characters.").max(50, "Last name must be at most 50 characters."),
  email: emailSchema,
  password: passwordSchema,
  ip: z.string().optional()
});
