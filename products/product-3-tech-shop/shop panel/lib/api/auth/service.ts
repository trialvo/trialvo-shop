...
class AuthService {
  ...
  async resetPassword(
    ...
    try {
      ...
      throw new Error(getServerErrorMessage(err, "Password reset failed"));
    }
  }
  ...
  async forgotPassword(
    ...
    try {
      ...
      throw new Error(getServerErrorMessage(err, "Failed to send reset OTP"));
    }
  }

  async getForgotPassMethods(): Promise<{ email: boolean; sms: boolean }> {
    try {
      const response = await api.get<{
        success?: boolean;
        email?: boolean;
        sms?: boolean;
      }>("/user/forgotPassMethods");
      return {
        email: response.data?.email !== false,
        sms: !!response.data?.sms,
      };
    } catch {
      // Safe default: email-only if methods endpoint is unavailable
