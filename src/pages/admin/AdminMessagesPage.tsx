 ...
 return (
  <div className="space-y-6">
   <div>
    ...
    <p className="text-sm text-slate-400">Customer contact messages</p>
   </div>
       ...
       <table className="w-full">
        <thead>
         <tr className="border-b border-slate-700/50">
          <th className="text-left text-xs text-slate-400 font-medium py-3 px-4">Sender</th>
          <th className="text-left text-xs text-slate-400 font-medium py-3 px-4">Subject</th>
        ...
        <tbody>
         {filtered?.map((msg) => (
          <tr
           key={msg.id}
           className={`border-b border-slate-700/30 last:border-0 hover:bg-slate-700/20 cursor-pointer ${!msg.is_read ? 'bg-primary/5' : ''
            }`}
           onClick={() => handleView(msg)}
          >
           <td className="py-3 px-4">
            <div>
             <p className={`text-sm ${!msg.is_read ? 'font-semibold text-white' : 'text-slate-300'}`}>
              {msg.name}
             </p>
           ...
           <td className="py-3 px-4">
            <p className={`text-sm ${!msg.is_read ? 'font-medium text-white' : 'text-slate-300'}`}>
             {msg.subject}
            </p>
           ...
           <td className="py-3 px-4">
            {msg.is_read ? (
             <Badge variant="outline" className="text-[11px] border-slate-600 text-slate-400">Read</Badge>
            ) : (
     ...
     {viewMessage && (
      <div className="space-y-4 mt-4">
       ...
       <div>
        ...
        <p className="bg-slate-700/50 p-4 rounded-lg mt-1 text-sm leading-relaxed whitespace-pre-wrap">
         {viewMessage.message}
        </p>
       </div>
      </div>
