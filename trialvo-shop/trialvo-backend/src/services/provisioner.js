...
/**
 * Option 1 (hosted): grant ADMIN access on the shared demo API/DB.
 * Shop URL comes from product.deploy_config (Lifestyle/Fashion/Tech UIs differ).
...
async function provisionHostedSharedDemo(requestRow, {
  ...
  const check = await ensureConfigured(deployConfig);
  if (!check.ok) {
    await pool.query(
  ...
  const urls = check.urls || getSharedDemoUrls(deployConfig);
  const demoDbName = check.database || deployConfig.shared_demo_db_name || 'lifestyle_demo';
  ...
  const meta = {
    note: 'Option 1 per-product demo — ADMIN access grant (no per-trial Docker)',
    sharedDemo: true,
    provisionMode: 'shared',
    productSlug: requestRow.product_slug || null,
    sharedDemoDbName: demoDbName,
    lifestyleAdminEmail: adminEmail,
    lifestyleAdminId: created.adminId,
    roleId: created.roleId,
    disclaimer: 'Shared demo — product data may be visible to other trial users on the same product stack',
  };
...
async function provisionFromRequest(requestRow, days = 14) {
  ...
  if (isHosted && !supportsTrialOption(deployConfig, 1)) {
    throw new Error(`Option 1 (hosted) is not enabled for product ${product.slug}`);
  }
  if (!isHosted && !supportsTrialOption(deployConfig, 2)) {
    throw new Error(`Option 2 (self-hosted) is not enabled for product ${product.slug}`);
  }
  let subdomain = null;
  let domain = null;
  let status;
  ...
  if (isHosted) {
    ...
    const result = await provisionHostedSharedDemo(
      { ...requestRow, product_slug: product.slug },
      {
    ...
    provisionMeta = result.meta || null;
  } else {
