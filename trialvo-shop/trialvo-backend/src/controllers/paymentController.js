  ...
  try {
    ...
    if (event === 'payment.success' && paymentStatus === 'paid') {
      try {
        ...
        if (result.ok) {
          console.log(`[IPN] Trial activation:`, {
            ...result,
            licenseKey: result.licenseKey ? '[redacted]' : null,
            packToken: result.packToken ? '[redacted]' : null,
          });
          if (result.packToken && order.customer_email) {
            const { emailPaidPackDelivery } = require('./licensePackController');
            await emailPaidPackDelivery({
              to: order.customer_email,
              name: order.customer_name,
              licenseKey: result.licenseKey,
              packToken: result.packToken,
              orderId: order.order_id || order.id,
            });
            console.log(`[IPN] Paid pack email queued → ${order.customer_email}`);
          }
        } else {
