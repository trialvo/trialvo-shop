...
async function createTestimonial(req, res, next) {
    try {
        const id = uuidv4();
        const { name, role, content, rating, avatar, images, is_active } = req.body;

        await pool.execute(
            `INSERT INTO testimonials (id, name, role, content, rating, avatar, images, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [
...
async function updateTestimonial(req, res, next) {
    try {
        ...
        const fields = [];
        const values = [];
        const jsonFields = ['name', 'role', 'content', 'images'];
