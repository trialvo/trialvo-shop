# Trialvo Pay — Demo & Testing Guide
...
3. ✅ Build & start Trialvo Pay server
...
Simulated online store that integrates with Trialvo Pay to test the complete payment flow.
...
5. Trialvo Pay payment page opens → Shows order summary, countdown timer
...
```
┌─────────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│   Test E-Commerce   │────▶│   Trialvo Pay API   │────▶│  EPS Sandbox     │
...
- **Rust** (cargo) — for building Trialvo Pay
...
# 2. Start Trialvo Pay (in terminal 1)
cargo run
...
```bash
# Stop Trialvo Pay & Test Shop — close the terminal windows, or:
taskkill /IM trialvo_pay.exe /F 2>nul
