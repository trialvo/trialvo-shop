// Trialvo Pay Merchant Portal — Pages
...
async integration() {
    ...
    const BASE = settings.api_base_url || 'https://pay.trialvo.com/api/v1';
...
def create_bill(amount, description, customer_name, customer_email, order_id=None):
    """Create a payment bill via Trialvo Pay API."""
    body = {
        "amount": str(amount),
        "currency": "BDT",
...
?>`
    ...
    <div class="card">
        ...
        <p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px">When Trialvo Pay sends a webhook to your endpoint, verify the signature using your webhook secret:</p>
        <div class="code-block">
            <button class="btn btn-sm btn-secondary btn-copy" onclick="MUI.copyToClipboard(this.closest('.code-block').querySelector('pre').textContent)"><i data-lucide="copy" style="width:12px;height:12px"></i> Copy</button>
            <pre>import hmac, hashlib

def verify_webhook(payload_body: bytes, signature_header: str, webhook_secret: str) -> bool:
    """Verify Trialvo Pay webhook signature."""
    expected = hmac.new(
    ...
    <div class="card">
        ...
        <a href="/docs" target="_blank" style="color:var(--accent)">Trialvo Pay Developer Documentation →</a></p>
    </div>`;
},
