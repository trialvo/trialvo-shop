@echo off
REM ============================================================
REM  PayVault Demo Setup — One-Click Test Environment
REM  Run this script to start everything needed for a client demo
REM ============================================================

echo.
echo  ╔══════════════════════════════════════════════════╗
echo  ║         PayVault — Demo Environment Setup        ║
echo  ╚══════════════════════════════════════════════════╝
echo.
...
REM --- Step 3: Build ^& Start PayVault ---
echo.
echo [3/5] Building ^& starting PayVault (this may take 1-2 minutes on first run)...
start "PayVault Server" cmd /c "cargo run 2>&1 | findstr /V DEBUG"
echo  ⏳ Waiting for PayVault to start...
:wait_payvault
timeout /t 3 /nobreak >nul
curl -s http://localhost:8080/health >nul 2>&1
if %errorlevel% neq 0 goto wait_payvault
echo  ✅ PayVault is running on http://localhost:8080
...
echo  ║    Email:    admin@payvault.trialvo.com           ║
