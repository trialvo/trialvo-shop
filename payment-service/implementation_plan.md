# PayVault — Centralized Payment Microservice
...
| **Container** | **Docker + Docker Compose** | Single `docker-compose.yml` for Postgres, Redis, and PayVault API. |
...
### 3.1 Service Authentication (Client → PayVault)
...
```sql
-- ═══════════════════════════════════════════════════════════════════
-- PayVault — Central Payment Service Database Schema
...
INSERT INTO system_config (category, key_name, value, value_type, description, is_secret) VALUES
    ...
    -- General
    ('general', 'base_url', 'https://payvault.trialvo.com', 'string', 'PayVault public base URL', FALSE),
    ('general', 'domain', 'payvault.trialvo.com', 'string', 'PayVault domain name', FALSE),
    ...
    ('smtp', 'from_email', 'noreply@payvault.trialvo.com', 'string', 'From email address', FALSE),
    ('smtp', 'from_name', 'PayVault', 'string', 'From display name', FALSE),
    -- SMS
    ...
    ('sms', 'sender_id', 'PayVault', 'string', 'SMS sender ID', FALSE),
    -- Security
...
1. SERVICE creates bill
   POST /api/v1/bills  ─────────►  PayVault generates bill_token
                                    Status: PENDING
...
3. SERVICE redirects customer to PayVault
   Customer browser ──► GET /pay/pvb_a1b2c3d4...
                         PayVault renders payment page

4. Customer clicks "Pay"
   POST /pay/pvb_a1b2c3d4.../initiate
   PayVault calls EPS InitializeEPS
   Status: PROCESSING
...
6. EPS redirects back
   ...
   PayVault calls EPS CheckMerchantTransactionStatus
   Status: PAID ✓

7. PayVault fires IPN to service
...
| EPS Request Field | Source in PayVault | Notes |
|---|---|---|
...
| `successUrl` | `{payvault_base}/pay/callback?type=success` | PayVault's own callback, NOT the service's |
| `failUrl` | `{payvault_base}/pay/callback?type=fail` | PayVault's own callback |
| `cancelUrl` | `{payvault_base}/pay/callback?type=cancel` | PayVault's own callback |
...
### 7.4 Callback Handling (EPS → PayVault)
...
**PayVault callback handler (`api/payment/callback.rs`):**
...
**Signature:** `X-PayVault-Signature: HMAC-SHA256(ipn_endpoint.secret, raw_json_body)`
...
```
Service/Admin                PayVault                      EPS (if applicable)
     │                          │                               │
     │ POST /api/v1/refunds     │                               │
     │ ─────────────────────►   │                               │
     │                          │ Validate:                     │
...
- **Production:** `https://payvault.trialvo.com`
- **API base:** `https://payvault.trialvo.com/api/v1/`
- **Admin panel:** `https://payvault.trialvo.com/admin/`
- **Payment page:** `https://payvault.trialvo.com/pay/{bill_token}`
- **EPS callbacks:** `https://payvault.trialvo.com/pay/callback?type={success|fail|cancel}`
...
services:
  payvault:
    ...
    env_file: .env
    environment:
      BASE_URL: https://payvault.trialvo.com
      RUST_LOG: info
...
### Nginx Config (payvault.trialvo.com)

```nginx
server {
    listen 80;
    server_name payvault.trialvo.com;
...
# Directory: /opt/payvault/
# Domain: payvault.trialvo.com (DNS A record → 155.248.253.24)
# Managed by: docker compose
# TLS: Let's Encrypt via certbot (auto-renew cron)
#   certbot certonly --standalone -d payvault.trialvo.com
...
| 4 | **Payment page branding** | **White-labeled per service** — shows service logo/name from `services.logo_url` + `services.display_name`, with "Powered by PayVault" + EPS branding footer |
| 5 | **Domain** | **`payvault.trialvo.com`** — DNS A record → `155.248.253.24`, TLS via Let's Encrypt |
