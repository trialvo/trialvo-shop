/**
 * ═══════════════════════════════════════════════════════════════
 *  Test E-Commerce — PayVault Integration Test Harness
 *  
 *  A simple Node.js server that:
 *   1. Serves a single-page e-commerce test app
 *   2. Makes HMAC-authenticated calls to PayVault /api/v1/*
 *   3. Receives IPN webhooks from PayVault
...
const PORT = 3456;
const PAYVAULT_BASE = 'http://localhost:8080';
...
async function handleCreateBill(req, res) {
  ...
  try {
    const result = await httpRequest('POST', `${PAYVAULT_BASE}/api/v1/bills`, headers, body);
...
async function handleGetBill(req, res) {
  ...
  try {
    const result = await httpRequest('GET', `${PAYVAULT_BASE}/api/v1/bills/${token}`, headers, null);
    json(res, result.status, result.body);
  } catch (err) {
    json(res, 500, { error: err.message });
  }
...
async function handleCancelBill(req, res) {
  ...
  try {
    const result = await httpRequest('DELETE', `${PAYVAULT_BASE}/api/v1/bills/${token}`, headers, null);
    json(res, result.status, result.body);
  } catch (err) {
    json(res, 500, { error: err.message });
  }
...
async function handleGetTransaction(req, res) {
  ...
  try {
    const result = await httpRequest('GET', `${PAYVAULT_BASE}/api/v1/transactions/${txId}`, headers, null);
    json(res, result.status, result.body);
  } catch (err) {
    json(res, 500, { error: err.message });
  }
...
async function handleCreateRefund(req, res) {
  ...
  try {
    const result = await httpRequest('POST', `${PAYVAULT_BASE}/api/v1/refunds`, headers, body);
    json(res, result.status, result.body);
  } catch (err) {
    json(res, 500, { error: err.message });
  }
...
async function handleGetRefund(req, res) {
  ...
  try {
    const result = await httpRequest('GET', `${PAYVAULT_BASE}/api/v1/refunds/${refundId}`, headers, null);
    json(res, result.status, result.body);
  } catch (err) {
    json(res, 500, { error: err.message });
  }
...
async function handleWebhook(req, res) {
  ...
  const eventType = req.headers['x-payvault-event'] || 'unknown';
  ...
  const entry = {
    ...
    headers: {
      'x-payvault-signature': signature,
      'x-payvault-event': eventType,
    },
  };
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // Webhook receiver
    if (pathname === '/webhooks/payvault' && req.method === 'POST') return handleWebhook(req, res);
...
server.listen(PORT, () => {
  console.log(`
╔══════════════════════════════════════════════════════════════╗
║          🛒 Test E-Commerce — PayVault Test Harness          ║
╠══════════════════════════════════════════════════════════════╣
║                                                              ║
║   Shop:      http://localhost:${PORT}                          ║
║   Webhook:   http://localhost:${PORT}/webhooks/payvault         ║
║   PayVault:  http://localhost:8080                            ║
║                                                              ║
║   Steps:                                                     ║
