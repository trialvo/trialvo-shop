/**
 * PayVault Admin SPA - Page Implementations
      ...
      <div class="auth-page">
        <div class="auth-card">
          <div class="auth-logo">
            ...
            <div>
              <div class="auth-title">PayVault</div>
          ...
          <form id="login-form" class="auth-form">
            <div class="form-group">
              ...
              <div class="input-icon-wrap">
                ...
                <input class="form-input" id="login-email" type="email" placeholder="admin@payvault.trialvo.com" required autofocus>
              </div>
            </div>
            <div class="form-group">
    ...
    try {
      ...
      Modal.show('⚠️ Save Your Webhook Secret', `
        <div class="alert-warning p-16 rounded-8 mb-16">
          ...
          Your server must use this to verify the <code>X-PayVault-Signature</code> header.
        </div>
        <div class="key-reveal">
          <div class="key-display">
  ...
  _addAdminModal() {
    Modal.show('Add Administrator', `
      <form id="add-admin-form">
        ${Components.renderInput('adm-email', 'Email', 'email', 'admin@payvault.trialvo.com', '', true)}
      ...
      <div class="page-content">
        <div class="page-header">
          <div class="page-title-wrap">
            ...
            <p class="page-subtitle">End-to-end PayVault system lifecycle — from onboarding to payment settlement</p>
          </div>
        </div>
        ...
        <div class="flow-page">
          ...
          <div class="card flow-section">
            ...
            <div class="flow-list" style="margin-top:24px">
              ...
              ${listItem(4, 'Integration is live', 'Merchant integrates using their Service ID + API Key. PayVault routes their bills through EPS.')}
            </div>
          </div>
          ...
          <div class="card flow-section">
            ...
            <div class="flow-diagram">
              ...
              ${arrow()}
              ${node('server', 'PayVault', 'Validates + creates bill', 'info')}
              ${arrow()}
            ...
            <div class="flow-diagram" style="margin-top:4px">
              ${node('shield-check', 'PayVault Verifies', 'check_status API', 'success')}
              ${arrow()}
          ...
          <div class="card flow-section">
            ...
            <div class="flow-diagram">
              ...
              ${arrow()}
              ${node('server', 'PayVault', 'Creates refund request', 'info')}
              ${arrow()}
          ...
          <div class="card flow-section">
            ...
            <p style="font-size:0.8125rem;color:var(--color-text-2);margin-bottom:20px">
              IPN (Instant Payment Notification) is PayVault's push-notification system. When a payment event occurs,
              PayVault signs a JSON payload with the merchant's webhook secret and POSTs it to every subscribed endpoint.
            ...
            <div style="margin-bottom:28px">
              ...
              <div class="flow-diagram">
                ...
                ${arrow()}
                ${node('lock', 'Sign Payload', 'X-PayVault-Signature', 'brand')}
                ${arrow()}
            ...
            <div style="margin-bottom:0">
              <div style="font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--color-text-3);margin-bottom:12px">Signature Verification — X-PayVault-Signature header</div>
              <p style="font-size:0.8125rem;color:var(--color-text-2);margin-bottom:14px">
                Every IPN POST includes the header <code style="font-size:0.78em;background:var(--color-surface-2);padding:2px 6px;border-radius:4px;color:var(--color-brand-2)">X-PayVault-Signature</code>.
              ...
              <div class="flow-list">
                ...
                ${listItem(4, 'Respond HTTP 2xx within 10 seconds', 'If your endpoint takes longer or returns a non-2xx, PayVault retries: 30s → 2m → 8m → 30m → 2h (max 5 retries). After 5 failures the delivery is marked exhausted.')}
              </div>
            </div>

            <div class="docs-callout warn" style="margin-top:20px">
              <div class="docs-callout-title"><i data-lucide="alert-triangle"></i> Never skip signature verification</div>
              <p>Any public URL that accepts IPN without verifying the <code>X-PayVault-Signature</code> header can be spoofed by a third party sending fake payment.success events. Always verify before fulfilling orders or crediting accounts.</p>
            </div>
          </div>
      ...
      <div class="page-content">
        ...
        <div class="admin-docs card" style="padding:32px">
          <div class="docs-section">
            ...
            <ul>
              <li>Merchant Portal URL: <code>https://payvault.trialvo.com/merchant</code></li>
            ...
            <ol>
              ...
              <li>PayVault sends an IPN to the merchant's webhooks: <code>refund.approved</code> or <code>refund.rejected</code></li>
            </ol>
            <div class="docs-callout info">
              <div class="docs-callout-title"><i data-lucide="info"></i> Actual refund processing</div>
              <p>PayVault marks the refund as approved, but the actual EPS refund transaction depends on your EPS gateway settings. Confirm with your EPS account whether refunds are processed automatically or require manual gateway action.</p>
            </div>
            ...
            <p>IPN (Instant Payment Notification) is how PayVault notifies merchants of payment events. As admin, you have <strong>oversight</strong> over all IPN endpoints across all merchants. Merchants manage their own webhooks — you cannot create or delete their endpoints, but you can monitor and troubleshoot.</p>

            <h3>What is an IPN?</h3>
            <p>When a bill changes status (payment succeeds, fails, is cancelled, or a refund is decided), PayVault immediately POSTs a signed JSON payload to all merchant webhook URLs that are subscribed to that event. This is called an IPN (Instant Payment Notification) — our equivalent of what Stripe calls a "webhook event".</p>
            ...
            <ol>
              ...
              <li>PayVault finds all active webhook endpoints for that merchant subscribed to the event</li>
              <li>PayVault signs the JSON payload using <code>HMAC-SHA256(raw_body, webhook_secret)</code></li>
              <li>PayVault POSTs to each endpoint with the <code>X-PayVault-Signature</code> header</li>
