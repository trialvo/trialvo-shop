  ...
  return (
    <div className="space-y-6">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="md:hidden p-3 space-y-3">
                {filtered?.map((t) => (
                  <div key={t.id} className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-4 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-sm flex-shrink-0">
                        {t.name.en.charAt(0)}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-white truncate">{t.name.en}</p>
                        <p className="text-xs text-gray-400 truncate">{t.role.en}</p>
                      </div>
                      <button onClick={() => handleToggleActive(t.id, !t.is_active)}>
                        {t.is_active ? (
                          <Badge variant="outline" className="text-[10px] border-emerald-500/20 text-emerald-400 bg-emerald-500/10">Active</Badge>
                        ) : (
                          <Badge variant="outline" className="text-[10px] border-white/10 text-gray-400">Inactive</Badge>
                        )}
                      </button>
                    </div>
                    <div className="flex items-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < t.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-600'}`} />
                      ))}
                    </div>
                    <div className="flex items-center justify-end gap-1 pt-2 border-t border-white/[0.06]">
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-gray-400 hover:text-white" onClick={() => openEdit(t)}>
                        <Pencil className="w-3.5 h-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-gray-400 hover:text-red-400" onClick={() => setDeleteId(t.id)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[#1e2030] border-white/10 text-white max-w-2xl max-h-[85vh] overflow-y-auto w-[95vw] sm:w-full">
          <DialogHeader>
